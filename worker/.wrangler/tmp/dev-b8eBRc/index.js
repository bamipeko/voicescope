...
// .wrangler/tmp/bundle-Br63gd/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-Br63gd/middleware-insertion-facade.js
init_modules_watch_stub();
...
async function verify(c) {
  try {
    ...
    const codeData = await c.env.CODES.get(`code:${normalizedCode}`, "json");
    ...
    const expiryISO = new Date(exp * 1e3).toISOString();
    ...
    await c.env.CODES.put(
      ...
      JSON.stringify({ activatedAt: (/* @__PURE__ */ new Date()).toISOString(), expiresAt: expiryISO, tier: codeData.tier }),
...
function isModelAllowed(tier, model) {
...
async function checkRateLimit(kv, code, tier, deviceHash) {
  const limit = RATE_LIMITS[tier] || 60;
  const hourKey = (/* @__PURE__ */ new Date()).toISOString().slice(0, 13).replace(/[-T:]/g, "");
...
__name(openaiProxy, "openaiProxy");

// src/routes/anthropic.ts
init_modules_watch_stub();
init_auth();
async function anthropicProxy(c) {
...
init_modules_watch_stub();
init_auth();
async function transcribeProxy(c) {
  const token = c.req.header("Authorization")?.replace("Bearer ", "");
  if (!token) return c.json({ error: "Unauthorized" }, 401);
  const payload = await verifyJWT(token, c.env.JWT_SECRET);
  if (!payload) return c.json({ error: "Invalid or expired token" }, 401);
...
__name(transcribeProxy, "transcribeProxy");
...
app.post("/v1/audio/transcriptions", async (c) => {
  ...
  const payload = await verifyJWT2(jwt, c.env.JWT_SECRET);
